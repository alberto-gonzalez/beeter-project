package edu.upc.eetac.dsa.agonzalez.beeter.api;
...
import edu.upc.eetac.dsa.agonzalez.beeter.api.model.Sting;
import edu.upc.eetac.dsa.agonzalez.beeter.api.model.StingCollection;
...
public class StingResource {
	...
	private String INSERT_STING_QUERY = "insert into stings (username, subject, content) values (?, ?, ?)";
	 //no puede aver ninguna linea enre los @ i la funcion
	@POST
	...
	public Sting createSting(Sting sting) {
		...
		try {
			...
			stmt.executeUpdate();
			ResultSet rs = stmt.getGeneratedKeys();//equivale al sting id
			if (rs.next()) {
